...
- **키보드 (의도 명시)**: `0–9` = 상단 0~9번 타일, `c` = cashout, `r` = random pick. UI 안내 1줄: *"1–0 = 상단 10칸 / R 랜덤 / C 캐쉬아웃"*.
...
- `handleReveal` bomb hit: **같은 tick에 `activeRound = null`** + history/lastOutcome 업데이트 (추가 강화: 새로고침 시 bomb 상태로 안 돌아옴).
...
- **상수 `CLIENT_SEED` 제거**. `placeMines({ clientSeed: minesStore.use(s=>s.clientSeed), ... })`.
- PF 모달:
...
### 종료 게이트
1. `bunx eslint . --max-warnings=0` — 0 warning
...
4. 수동 QA 6항목:
   ...
   - **bomb hit 후 새로고침** → idle 상태로 시작(보드 비어있음, activeRound null 확인)
...
### 신규 테스트 (총 71 → 73)
- `MinesScreen.persist.spec.tsx` — 기존 v1 JSON(activeRound/clientSeed 없음) 로드 → 머지 후 기본값 주입.
- `MinesScreen.restore.spec.tsx` — `activeRound != null` 마운트 시 `tryDebit`/`liveBetsStore.push` 호출 **0회**, `round.phase === "playing"` 진입.
...
**Modified (3)**
- `src/features/games/mines/MinesScreen.tsx` — 인터랙션·키보드·랜덤픽·복원·툴팁·PF 시드 입력. `CLIENT_SEED` 상수 제거. **본문 250줄 이하** 목표 (MinesTile 분리).
- `src/shared/games/state/persistedGameState.ts` — `MinesPersisted`에 `activeRound`·`clientSeed` 추가, initial 기본값. **version=1 유지**.
- `src/shared/games/rules/gameRules.ts` — MINES_RULES에 단축키 1줄 추가.

**Created (3)**
- `src/features/games/mines/MinesTile.tsx` — `React.memo`, flip/gem/bomb 캡슐화.
- `src/features/games/mines/__tests__/MinesScreen.persist.spec.tsx`
- `src/features/games/mines/__tests__/MinesScreen.restore.spec.tsx`
...
### 라운드 종료 보고 양식 (PROMPT_HEADER 준수)
- 변경 파일 목록 (Modified 3 / Created 3)
- 비대상 준수 Y/N (8항목 체크리스트)
- `bunx eslint . --max-warnings=0` 결과
- `bunx vitest run` 결과 (73/73)
- `bun run build` 결과
- 수동 회귀: Mines 6항목 + 로비
- `supabase/` 변경 0건 확인
