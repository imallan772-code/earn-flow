...
export function EventDetail({ id }: { id: string }) {
  ...
  const cd = useCountdown(e?.status === "예정" ? e.startsAt : e?.endsAt ?? new Date().toISOString());
  ...
  if (!e) {
    return (
      <div className="glass-2 rounded-2xl p-8 text-center text-sm text-[var(--color-muted)]">
        이벤트를 찾을 수 없습니다.
        <div className="mt-3">
          <Link to="/event" className="text-[var(--color-cyan)]">목록으로</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        <span className="rounded-full bg-black/35 px-2 py-0.5 text-[10px] font-bold backdrop-blur">{e.status}</span>
      ...
      {e.status !== "종료" && (
        <div className="glass-3 grid grid-cols-4 gap-2 rounded-2xl p-3">
          {[["일", cd.d], ["시간", cd.h], ["분", cd.m], ["초", cd.s]].map(([l, v]) => (
            <div key={l as string} className="flex flex-col items-center rounded-xl bg-black/25 py-3">
              <div className="font-numeric text-2xl font-extrabold tabular-nums">{String(v).padStart(2, "0")}</div>
      ...
      <div className="glass-2 rounded-2xl p-4">
        <div className="mb-2 flex items-center justify-between text-xs">
          <span className="flex items-center gap-1 text-[var(--color-muted)]">
            ...
            {e.cap && <span className="text-[var(--color-muted-2)]"> / {e.cap.toLocaleString()}</span>}
          </span>
          <span className="font-numeric font-bold">{pct}%</span>
        </div>
        <div className="h-2 overflow-hidden rounded-full bg-black/40">
          <div className="h-full rounded-full bg-holographic transition-all duration-700" style={{ width: `${pct}%` }} />
        </div>
      </div>

      <div className="glass-2 rounded-2xl p-4">
        <h2 className="mb-2 text-sm font-bold">이벤트 안내</h2>
        <p className="whitespace-pre-wrap text-sm leading-relaxed text-[var(--color-muted)]">{e.body}</p>
      </div>
        ...
        <div className="glass-2 rounded-2xl p-4">
          ...
          <ul className="flex flex-col gap-1.5">
            {e.leaderboard.map((row) => (
              <li key={row.rank} className="flex items-center gap-2 rounded-xl bg-black/25 px-3 py-2 text-sm">
                <span
                ...
                <span className="font-numeric text-xs font-bold text-[var(--color-emerald)]">+{row.score}%</span>
              </li>
            ))}
          </ul>
        </div>
