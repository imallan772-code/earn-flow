    ...
    <div className="relative min-h-dvh w-full overflow-hidden bg-cosmic">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-md flex-col gap-5 px-5 pb-10 pt-6 safe-top">
        ...
        <div className="flex gap-2.5">
          <Link
            to="/signup"
            className="group flex h-14 flex-1 items-center justify-center gap-2 rounded-2xl bg-holographic font-bold text-(--color-bg-0) shadow-glow-purple"
          >
            지금 무료 시작
          ...
          {MOCK_LANDING_HERO_STATS.map((s) => {
            ...
            return (
              <Premium3DCard key={s.label} className="p-3">
                <div className="text-[10px] font-medium text-(--color-muted)">{s.label}</div>
                <div
                ...
                <div className="mt-0.5 text-[10px] text-muted-2">{s.sub}</div>
              </Premium3DCard>
            );
          })}
        </div>
        ...
        <div className="mt-4 text-center text-[11px] text-muted-2">
