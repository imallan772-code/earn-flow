-- =============================================================================
-- L-2: refund_phon_for_bet_v2 + game_rounds.refunded_at
-- Mutual exclusion: credit_phon_for_payout_v2 rejects refunded rounds
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- refund_phon_for_bet_v2 — mid-round cancel (PF / unmount)
-- Same round_id as debit_phon_for_bet_v2 (NOT a -refund suffix)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- credit_phon_for_payout_v2 — reject refunded rounds (AC-8)
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_gr.refunded_at IS NOT NULL THEN
    RAISE EXCEPTION 'MONEY_ROUND_REFUNDED' USING ERRCODE = 'P0002',
      HINT = 'round was refunded; payout not allowed';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.refund_phon_for_bet_v2(bigint, text, text) TO authenticated;
